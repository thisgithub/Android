 ...
 */
package com.test.android.push.xmpp.net;
...
import com.test.android.push.org.dom4j.io.XMPPPacketReader;
import com.test.android.push.org.dom4j.io.jivesoftware.openfire.net.MXParser;
import com.test.android.push.util.Config;
import com.test.android.push.xmpp.router.PacketRouter;
...
public class StanzaHandler {
    ...
    private void processMessage(Element doc) {
        ...
        log.debug("processMessage() end...");
    }

    private void processPresence(Element doc) {
        log.debug("processPresence() start...");
        Presence packet;
        try {
    ...
    private void processIQ(Element doc) {
        ...
        router.route(packet);
        session.incrementClientPacketCount();
        log.debug("processIQ() end...");
    }

    private IQ getIQ(Element doc) {
    	log.debug("getIQ() start...");
    ...
    private void createSession(XmlPullParser xpp)
        ...
        log.debug("createSession() end...");
    }

    private boolean negotiateTLS() {
    	log.debug("negotiateTLS() start...");
    ...
    private void startTLS() throws Exception {
        ...
        log.debug("startTLS() end...");
    }

    private void tlsNegotiated() {
    	log.debug("tlsNegotiated() start...");
        ...
        sb.append("</stream:features>");
        connection.deliverRawText(sb.toString());
        log.debug("tlsNegotiated() end...");
    }

    private String randomString(int length) {
    	log.debug("randomString() start...");
        if (length < 1) {
        	log.debug("randomString() end...");
            return null;
        }
